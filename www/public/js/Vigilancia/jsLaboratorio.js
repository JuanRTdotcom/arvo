		...
		{	width: 300,
			...
			render: function (data, type, full, meta) {
				...
				return `
					...
					<p class="miIcoEnf ${miClaseS}"> ${data.Nresultado}</p> 
	...
	document.getElementById('btn_limpiarFichaFiltros').addEventListener('click',e => {
		e.preventDefault()
			...
			.then( dd => {
				...
				$("#enfermedad_f").select2({}).val('').trigger("change");
				$("#prueba_f").select2({}).val('').trigger("change");
				$("#resultado_f").select2({}).val('').trigger("change");
