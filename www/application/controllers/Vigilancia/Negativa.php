<?php

defined('BASEPATH') or exit('No direct script access allowed');

class Negativa extends CI_Controller
{

	public function __construct()
	{
		parent::__construct();

		/*cache control*/
		$this->output->set_header('Last-Modified: ' . gmdate("D, d M Y H:i:s") . ' GMT');
		$this->output->set_header('Cache-Control: no-store, no-cache, must-revalidate, post-check=0, pre-check=0');
		$this->output->set_header('Pragma: no-cache');
		$this->output->set_header("Expires: Mon, 26 Jul 2010 05:00:00 GMT");
		date_default_timezone_set('America/Lima');
		if ($this->session->userdata('sesionIniciada') != true) {
			redirect('login', 'refresh');
		}
		$this->load->library('form_validation');
		$this->load->helper('menu');
		$this->data['aplicacion'] = 'Noti';
	}

	const HEADER_STATUS_STRINGS = [
		'405' => 'HTTP/1.1 405 Method Not Allowed',
		'400' => 'BAD REQUEST',
		'408' => 'Request Timeout',
		'404' => 'NOT FOUND',
		'401' => 'UNAUTHORIZED',
		'200' => 'OK',
	];

	public function api_return($data = NULL, $http_code = NULL)
	{
		ob_start();
		header('content-type:application/json; charset=UTF-8');
		header(self::HEADER_STATUS_STRINGS[$http_code], true, $http_code);
		print_r(json_encode($data));
		ob_end_flush();
	}


	public function index()
	{
		$data['title'] 			= 'Arvovirosis';
		$data['titulo'] 		= 'Bienvenido';
		$data['page_name'] 		= "Vigilancia/Negativa_view.php";
		$data['page'] 			= "Ingreso de fichas de vigilancia";

		$this->load->view('plantilla', $data); 
	}

	public function Agregar()
	{
		$data['title'] 			= 'Nuevo Arvovirosis';
		$data['titulo'] 		= 'Bienvenido';
		$data['page_name'] 		= "Vigilancia/Nueva_Ficha_Negativa_view.php";
		$data['page'] 			= "Ingreso de fichas de vigilancia";

		$this->load->view('plantilla', $data);
	}

	public function Ver()
	{
		$idFicha = $this->uri->segment(4);
		$data['title'] 			= 'Nuevo Arvovirosis';
		$data['titulo'] 		= 'Bienvenido';
		$data['page_name'] 		= "Vigilancia/Ver_Ficha_Negativa_view.php";
		$data['page'] 			= "Ingreso de fichas de vigilancia";
		$miFicha 				= $this->obtenerinformacionFichaNegativa($idFicha);
		$data['inf_ficha'] 		= $miFicha[0];

		$this->load->view('plantilla', $data);
	}

	public function Editar()
	{
		$idFicha = $this->uri->segment(4);
		$data['title'] 			= 'Nuevo Arvovirosis';
		$data['titulo'] 		= 'Bienvenido';
		$data['page_name'] 		= "Vigilancia/Editar_Ficha_Negativa_view.php";
		$data['page'] 			= "Ingreso de fichas de vigilancia";
		$miFicha 				= $this->obtenerinformacionFichaNegativa($idFicha);
		$data['inf_ficha'] 		= $miFicha[0];

		$this->load->view('plantilla', $data);
	}

	public function obtenerinformacionFichaNegativa($id){
		$this->load->model('Negativa_model', 'negativa_model');
		$depts = $this->negativa_model->getFicha_x_Id($id);
		if ($depts['estado']) {
			return $depts['data'];
			// $this->api_return(['status' => true, 'departamentos' => $depts['data']], 200);
		} else {
			return null;
			// $this->api_return(['status' => false, 'tipo' => 'error_bd', 'mensaje' => json_encode($depts)], 200);
		}
	}

	public function listarFichasNegativas()
	{
		$this->load->model('Negativa_model', 'negativa_model');
		$data = $this->negativa_model->getFichasNegativas();
		if ($data['estado']) {
			$this->api_return(['status' => true, 'data' => $data['data']], 200);
		} else {
			$this->api_return(['status' => false, 'tipo' => 'error_bd', 'mensaje' => json_encode($data)], 200);
		}
	}

	public function registrarFichaNegativa()
	{
		$responsable_epidemiologia = $this->input->post("responsable_epidemiologia");
		$fecha_notificacion = $this->input->post("fecha_notificacion");
		$observacion = $this->input->post("observacion");
		$esNegativa = 1; // 0 - no es negativa    1- es negativa
		
		$this->load->model('Negativa_model', 'negativa_model');
		$data = $this->negativa_model->agregarFichaNegativa(
			$esNegativa,
			$responsable_epidemiologia,
			$fecha_notificacion,			
			$observacion
		);
		if ($data['estado']) {
			$this->api_return(['status' => true, 'data' => $data['data']], 200);
		} else {
			$this->api_return(['status' => false, 'tipo' => 'error_bd', 'mensaje' => json_encode($data)], 200);
		}
	}
	
	public function editarFichaNegativa()
	{
		$id = $this->input->post("id");
		$responsable_epidemiologia = $this->input->post("responsable_epidemiologia");
		$fecha_notificacion = $this->input->post("fecha_notificacion");
		$observacion = $this->input->post("observacion");
		
		$this->load->model('Negativa_model', 'negativa_model');
		$data = $this->negativa_model->editarFichaNegativa(
			$id,
			$responsable_epidemiologia,
			$fecha_notificacion,			
			$observacion
		);
		if ($data['estado']) {
			$this->api_return(['status' => true, 'data' => $data['data']], 200);
		} else {
			$this->api_return(['status' => false, 'tipo' => 'error_bd', 'mensaje' => json_encode($data)], 200);
		}
	}
}
